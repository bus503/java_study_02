...
//		do_search(stdArr);
	...
	private static void prn_Object(Student obj) {
	...
	private static void do_search(Student[] stdArr) {
		//검색
		Arrays.sort(stdArr);// comparabla compareto()에 의해 정렬
		prnStds(stdArr);
		Student findStd = new Student(6); //위치가 없으면 - 출력됨
...
//			//equals는 오브젝트의 equals
//			//이렇게 하면 다른학생이 나와버림 이유는 std01과 std02의 실제값을 비교하는게 아니라
//			// 참조된 주소값을 비교하기 때문에 false가 나옴
//		}
//		
		...
		// 이렇게 주소값을 알수있다 이 두개가 다르기 때문에 위에서 false가 나옴
		// 해시코드앤드 이콜즈로 오버라이딩해주면 같은학생이 나오게됨
		// 해시코드로 숫자화시키면 숫자가 똑같아지니까
	}
}
